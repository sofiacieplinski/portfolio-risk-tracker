...
def fetch_prices(tickers: "list[str]", period: str = PERIOD) -> pd.DataFrame:
    """
    Return a DataFrame of adjusted close prices, columns = tickers.
    Handles both single- and multi-ticker yfinance responses.
    Retries once on transient DB-lock errors from yfinance's sqlite cache.
    """
    import time as _time

    unique = list(dict.fromkeys(tickers))   # deduplicate, preserve order
    ...
    # Normalise to a flat DataFrame with ticker columns
    if len(unique) == 1:
        if "Close" in raw.columns:
            prices = raw[["Close"]].rename(columns={"Close": unique[0]})
        else:
            # MultiIndex fallback
            prices = raw["Close"].to_frame(name=unique[0])
    else:
        close = raw["Close"]
...
def show_dashboard(m: dict) -> None:
    ...
    console.print(
        ...
        f"Data: {PERIOD} ({m['n_days']} trading days).[/dim]"
    )
    console.print()
